...
try:
    ...
    if inspect.isfunction(difflib.get_close_matches):
        raise ImportError("Embedded difflib is not compiled to a fast binary, using the stdlib instead.")
...
def compress_tokens(tokens):
    ...
    result = [tokens[0]]
    for tok in tokens[1:]:
        if not tok.pre_tags and not result[-1].post_tags and result[-1].annotation == tok.annotation:
...
any_block_level_tag = cython.declare(tuple, tuple(sorted(block_level_tags | block_level_container_tags)))
