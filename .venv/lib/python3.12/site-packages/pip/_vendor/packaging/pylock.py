...
def _get(d: Mapping[str, Any], expected_type: type[_T], key: str) -> _T | None:
    ...
    if not isinstance(value, expected_type):
        raise PylockValidationError(
            f"Unexpected type {type(value).__name__} " f"(expected {expected_type.__name__})",
            context=key,
        )
    return value
...
def _get_sequence(d: Mapping[str, Any], expected_item_type: type[_T], key: str) -> Sequence[_T] | None:
    ...
    for i, item in enumerate(value):
        if not isinstance(item, expected_item_type):
            raise PylockValidationError(
                f"Unexpected type {type(item).__name__} " f"(expected {expected_item_type.__name__})",
                context=f"{key}[{i}]",
            )
    return value
...
class Package:
    ...
    def _from_dict(cls, d: Mapping[str, Any]) -> Self:
        ...
        try:
            for i, attestation_identity in enumerate(package.attestation_identities or []):  # noqa: B007
