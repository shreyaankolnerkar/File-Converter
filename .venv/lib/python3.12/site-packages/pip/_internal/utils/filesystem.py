...
def _subdirs_without_generic(path: str, predicate: Callable[[str, list[str]], bool]) -> Generator[Path]:
...
def subdirs_without_wheels(path: str) -> Generator[Path]:
    ...
    return _subdirs_without_generic(path, lambda root, filenames: any(x.endswith(".whl") for x in filenames))
