...
def _IsHelpShortcut(component_trace, remaining_args):
    ...
    if show_help:
        ...
        print(
            f"INFO: Showing help with the command {shlex.quote(command)}.\n",
            file=sys.stderr,
        )
    return show_help
...
def _PrintResult(component_trace, verbose=False, serialize=None):
    ...
    if serialize:
        if not callable(serialize):
            raise FireError("The argument `serialize` must be empty or callable:", serialize)
...
def _DisplayError(component_trace):
    ...
    if show_help:
        ...
        print(
            formatting.Error("ERROR: ") + component_trace.elements[-1].ErrorAsStr(),
            file=sys.stderr,
        )
...
def _Fire(component, args, parsed_flag_args, context, name=None):
    ...
    while True:
        ...
        if not handled and is_callable_object:
            ...
            try:
                component, remaining_args = _CallAndUpdateTrace(
                    component, remaining_args, component_trace, treatment="callable"
                )
                handled = True
            except FireError as error:
                candidate_errors.append((error, initial_args))
