...
def FindExecutableOnPath(executable, path=None, pathext=None, allow_extensions=False):
    ...
    if os.path.dirname(executable):
        raise ValueError(
            "FindExecutableOnPath({0},...) failed because first " "argument must not have a path.".format(executable)
        )
    ...
    if path is None:
        ...
        effective_path = path
    effective_pathext = (
        pathext if pathext is not None else _PlatformExecutableExtensions(platforms.OperatingSystem.Current())
    )
