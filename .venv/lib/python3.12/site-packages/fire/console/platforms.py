...
class InvalidEnumValue(Error):  # pylint: disable=g-bad-exception-name
    ...
    def __init__(self, given, enum_type, options):
        ...
        super(InvalidEnumValue, self).__init__(
            "Could not parse [{0}] into a valid {1}.  Valid values are [{2}]".format(
                given, enum_type, ", ".join(options)
            )
        )
        ...
        def __lt__(self, other):
            return (
                self._CmpHelper(
                    ...
                    (other.id, other.name, other.file_name),
                )
                < 0
            )
...
"""
    ...
    def IsCompatible(self, allow_py3=False, raise_exception=False):
        ...
        if not self.version:
            ...
            error = (
                "ERROR: Your current version of Python is not compatible with "
