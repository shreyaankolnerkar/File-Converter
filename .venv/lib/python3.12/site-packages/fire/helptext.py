...
def HelpText(component, trace=None, verbose=False):
    ...
    return "\n\n".join(_CreateOutputSection(name, content) for name, content in valid_sections)
...
def _SynopsisSection(component, actions_grouped_by_kind, spec, metadata, trace=None) -> tuple[str, str]:
...
def _ArgsAndFlagsSections(info, spec, metadata):
    ...
    kwonly_flag_items = [
        _CreateKeywordOnlyFlagItem(flag, docstring_info, spec, short_arg=flag[0] in unique_short_kwonly_flags)
    ...
    for action_group in actions_grouped_by_kind:
        if action_group.members:
            availability_line = _CreateAvailabilityLine(
                header=f"available {action_group.plural}:", items=action_group.names
            )
...
def _GetCallableUsageItems(spec, metadata):
    ...
    if not accepts_positional_args:
        items = [f"--{arg}={arg.upper()}" for arg in args_with_no_defaults]
    else:
...
def _GetCallableAvailabilityLines(spec):
    ...
    optional_flags = [
        f"--{flag}" for flag in itertools.chain(args_with_defaults, _KeywordOnlyArguments(spec, required=False))
    ]
    required_flags = [f"--{flag}" for flag in _KeywordOnlyArguments(spec, required=True)]

    # Flags section:
    availability_lines = []
    if optional_flags:
        availability_lines.append(
            _CreateAvailabilityLine(header="optional flags:", items=optional_flags, header_indent=2)
        )
    if required_flags:
        availability_lines.append(
            _CreateAvailabilityLine(header="required flags:", items=required_flags, header_indent=2)
        )
    if spec.varkw:
