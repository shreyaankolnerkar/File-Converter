...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        base_indices = [
            ...
            if i < num_base_records and any(base_records[i].BaseAnchor[j] is not None for j in class_indices)
        ]
        if not base_indices:
            return False
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        mark2_indices = [
            ...
            if i < num_mark2_records and any(mark2_records[i].Mark2Anchor[j] is not None for j in class_indices)
        ]
...
def prune_features(self, feature_index_map):
    self.ensureDecompiled()
    self.SubstitutionRecord = [r for r in self.SubstitutionRecord if r.FeatureIndex in feature_index_map.keys()]
    # remap feature indices
    for r in self.SubstitutionRecord:
...
def prune_features(self, feature_index_map):
    ...
    self.FeatureIndex = [feature_index_map[f] for f in self.FeatureIndex if f in feature_index_map.keys()]
    self.FeatureCount = len(self.FeatureIndex)
    return bool(self.FeatureCount or self.ReqFeatureIndex != 65535)
...
def prune_features(self, feature_index_map, keepEmptyDefaultLangSys=False):
    if (
        ...
        self.DefaultLangSys = None
    self.LangSysRecord = [l for l in self.LangSysRecord if l.LangSys.prune_features(feature_index_map)]
    self.LangSysCount = len(self.LangSysRecord)
    return bool(self.LangSysCount or self.DefaultLangSys)
