...
def _add_fvar(font, axes, instances: List[InstanceDescriptor]):
    ...
    for instance in instances:
        ...
        subfamilyNameID = nameTable.findMultilingualName(localisedStyleName, windows=True, mac=macNames)
...
def _unset_inconsistent_use_my_metrics_flags(vf, master_fonts):
    ...
    for glyph_name in glyf.keys():
        ...
        flagged_components = [
            (i, comp.glyphName) for i, comp in enumerate(glyph.components) if (comp.flags & USE_MY_METRICS)
        ]
        ...
        if any(
            _has_inconsistent_use_my_metrics_flag(master_glyf, glyph_name, flagged_components, expected_num_components)
...
def _add_HVAR(font, masterModel, master_ttfs, axisTags):
    getAdvanceMetrics = partial(_get_advance_metrics, font, masterModel, master_ttfs, axisTags, HVAR_FIELDS)
    _add_VHVAR(font, axisTags, HVAR_FIELDS, getAdvanceMetrics)


def _add_VVAR(font, masterModel, master_ttfs, axisTags):
    getAdvanceMetrics = partial(_get_advance_metrics, font, masterModel, master_ttfs, axisTags, VVAR_FIELDS)
