...
class Sections(Sequence[Section]):
    ...
    def __getitem__(self, key: int | slice) -> Section | List[Section]:
        if isinstance(key, slice):
            return [Section(sectPr, self._document_part) for sectPr in self._document_elm.sectPr_lst[key]]
...
class _Header(_BaseHeaderFooter):
    ...
    def _prior_headerfooter(self):
        ...
        preceding_sectPr = self._sectPr.preceding_sectPr
        return None if preceding_sectPr is None else _Header(preceding_sectPr, self._document_part, self._hdrftr_index)
