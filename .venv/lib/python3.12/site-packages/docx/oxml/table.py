...
class CT_Height(BaseOxmlElement):
    ...
    val: Length | None = OptionalAttribute("w:val", ST_TwipsMeasure)  # pyright: ignore[reportAssignmentType]
...
class CT_TblLayoutType(BaseOxmlElement):
    ...
    type: str | None = OptionalAttribute("w:type", ST_TblLayoutType)  # pyright: ignore[reportAssignmentType]
...
class CT_TblPr(BaseOxmlElement):
    ...
    jc: CT_Jc | None = ZeroOrOne("w:jc", successors=_tag_seq[8:])  # pyright: ignore[reportAssignmentType]
...
class CT_Tc(BaseOxmlElement):
    ...
    def grid_offset(self) -> int:
        ...
        preceding_tc_grid_spans = sum(tc.grid_span for tc in self.xpath("./preceding-sibling::w:tc"))
    ...
    def _grow_to(self, width: int, height: int, top_tc: CT_Tc | None = None):
        ...
        def vMerge_val(top_tc: CT_Tc):
            return ST_Merge.CONTINUE if top_tc is not self else None if height == 1 else ST_Merge.RESTART
    ...
    def _new_tbl(self) -> None:
        raise NotImplementedError("use CT_Tbl.new_tbl() to add a new table, specifying rows and columns")
...
class CT_TcPr(BaseOxmlElement):
    ...
    tcW: CT_TblWidth | None = ZeroOrOne("w:tcW", successors=_tag_seq[2:])  # pyright: ignore[reportAssignmentType]
    gridSpan: CT_DecimalNumber | None = ZeroOrOne(  # pyright: ignore[reportAssignmentType]
        "w:gridSpan", successors=_tag_seq[3:]
    )
    vMerge: CT_VMerge | None = ZeroOrOne("w:vMerge", successors=_tag_seq[5:])  # pyright: ignore[reportAssignmentType]
