...
class BaseSimpleType:
    ...
    def validate_int_in_range(cls, value: int, min_inclusive: int, max_inclusive: int) -> None:
        ...
        if value < min_inclusive or value > max_inclusive:
            raise ValueError(
                "value must be in range %d to %d inclusive, got %d" % (min_inclusive, max_inclusive, value)
            )
...
class XsdBoolean(BaseSimpleType):
    ...
    def convert_from_xml(cls, str_value: str) -> bool:
        if str_value not in ("1", "0", "true", "false"):
            raise InvalidXmlError("value must be one of '1', '0', 'true' or 'false', got '%s'" % str_value)
    ...
    def validate(cls, value: Any) -> None:
        if value not in (True, False):
            raise TypeError("only True or False (and possibly None) may be assigned, got '%s'" % value)
        ...
        def parse_xsd_datetime(dt_str: str) -> dt.datetime:
            ...
            if dt_str.endswith("Z"):
                try:
                    ...
                    return dt.datetime.strptime(dt_str, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=dt.timezone.utc)
                except ValueError:
                    return dt.datetime.strptime(dt_str, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=dt.timezone.utc)
...
class ST_HexColor(BaseStringType):
    @classmethod
    def convert_from_xml(cls, str_value: str) -> RGBColor | str:  # pyright: ignore[reportIncompatibleMethodOverride]
        if str_value == "auto":
            return ST_HexColorAuto.AUTO
        return RGBColor.from_string(str_value)

    @classmethod
    def convert_to_xml(cls, value: RGBColor) -> str:  # pyright: ignore[reportIncompatibleMethodOverride]
    ...
    def validate(cls, value: Any) -> None:
        ...
        if not isinstance(value, RGBColor):
            raise ValueError("rgb color value must be RGBColor object, got %s %s" % (type(value), value))
    ...
    def convert_from_xml(cls, str_value: str) -> bool:
        if str_value not in ("1", "0", "true", "false", "on", "off"):
            raise InvalidXmlError(
                "value must be one of '1', '0', 'true', 'false', 'on', or 'o" "ff', got '%s'" % str_value
            )
