...
class CT_HdrFtrRef(BaseOxmlElement):
    ...
    type_: WD_HEADER_FOOTER = RequiredAttribute("w:type", WD_HEADER_FOOTER)  # pyright: ignore[reportAssignmentType]
...
class CT_PageMar(BaseOxmlElement):
    ...
    top: Length | None = OptionalAttribute("w:top", ST_SignedTwipsMeasure)  # pyright: ignore[reportAssignmentType]
    right: Length | None = OptionalAttribute("w:right", ST_TwipsMeasure)  # pyright: ignore[reportAssignmentType]
    bottom: Length | None = OptionalAttribute(  # pyright: ignore[reportAssignmentType]
        "w:bottom", ST_SignedTwipsMeasure
    )
    left: Length | None = OptionalAttribute("w:left", ST_TwipsMeasure)  # pyright: ignore[reportAssignmentType]
    header: Length | None = OptionalAttribute("w:header", ST_TwipsMeasure)  # pyright: ignore[reportAssignmentType]
    footer: Length | None = OptionalAttribute("w:footer", ST_TwipsMeasure)  # pyright: ignore[reportAssignmentType]
    gutter: Length | None = OptionalAttribute("w:gutter", ST_TwipsMeasure)  # pyright: ignore[reportAssignmentType]


class CT_PageSz(BaseOxmlElement):
    """``<w:pgSz>`` element, defining page dimensions and orientation."""

    w: Length | None = OptionalAttribute("w:w", ST_TwipsMeasure)  # pyright: ignore[reportAssignmentType]
    h: Length | None = OptionalAttribute("w:h", ST_TwipsMeasure)  # pyright: ignore[reportAssignmentType]
...
class CT_SectPr(BaseOxmlElement):
    ...
    type: CT_SectType | None = ZeroOrOne("w:type", successors=_tag_seq[3:])  # pyright: ignore[reportAssignmentType]
    pgSz: CT_PageSz | None = ZeroOrOne("w:pgSz", successors=_tag_seq[4:])  # pyright: ignore[reportAssignmentType]
    pgMar: CT_PageMar | None = ZeroOrOne("w:pgMar", successors=_tag_seq[5:])  # pyright: ignore[reportAssignmentType]
    titlePg: CT_OnOff | None = ZeroOrOne("w:titlePg", successors=_tag_seq[14:])  # pyright: ignore[reportAssignmentType]
    del _tag_seq
    ...
    def get_headerReference(self, type_: WD_HEADER_FOOTER) -> CT_HdrFtrRef | None:
        ...
        matching_headerReferences = self.xpath("./w:headerReference[@w:type='%s']" % WD_HEADER_FOOTER.to_xml(type_))
...
class CT_SectType(BaseOxmlElement):
    ...
    val: WD_SECTION_START | None = OptionalAttribute("w:val", WD_SECTION_START)  # pyright: ignore[reportAssignmentType]
