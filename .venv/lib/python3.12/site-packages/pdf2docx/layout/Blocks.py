...
from ..common.docx import delete_paragraph, reset_paragraph_format
from ..common.share import BlockType, is_list_item, lower_round, rgb_value
...
class Blocks(ElementCollection):
    ...
    def lattice_table_blocks(self):
        ...
        return list(
            filter(
                lambda block: isinstance(block, Block) and block.is_lattice_table_block,
                self._instances,
            )
        )
    ...
    def stream_table_blocks(self):
        ...
        return list(
            filter(
                lambda block: isinstance(block, Block) and block.is_stream_table_block,
                self._instances,
            )
        )
    ...
    def table_blocks(self):
        ...
        return list(
            filter(
                lambda block: isinstance(block, Block) and block.is_table_block,
                self._instances,
            )
        )
    ...
    def inline_image_blocks(self):
        ...
        return list(
            filter(
                lambda block: isinstance(block, Block) and block.is_inline_image_block,
                self._instances,
            )
        )
    ...
    def text_blocks(self):
        ...
        return list(
            filter(
                lambda block: isinstance(block, Block) and block.is_text_image_block,
                self._instances,
            )
        )
    ...
    def restore(self, raws: list):
        ...
        for raw_block in raws:
            ...
            # table block
                ...
                BlockType.LATTICE_TABLE.value,
                BlockType.STREAM_TABLE.value,
            ):
    ...
    def clean_up(self, float_image_ignorable_gap: float, line_overlap_threshold: float):
        ...
        * remove empty lines
        ...
        for block in self._instances:
            if not isinstance(block, (ImageBlock, TextBlock)):
                continue
        ...
        f = (
            lambda line: line.bbox.intersects(page_bbox)
            and not line.white_space_only
            and (line.is_horizontal_text or line.is_vertical_text)
        )
        ...
        self.reset(instances)._identify_floating_images(float_image_ignorable_gap)._remove_overlapped_lines(
        ...
        * block is contained in potential shading
        ...
        def contained_in_shadings(block):
            for shading in shadings_exclude_white:
                if shading.contains(block, threshold=constants.FACTOR_MOST):
                    return True
            return False
    ...
    def make_docx(self, doc):
        ...
        for block in self._instances[::-1]:
            ...
            if block.is_float_image_block:
                continue

            # nothing to do if not end with table block
            if not block.is_table_block:
                break
        ...
        for block in self._instances:
            ...
            # check two adjacent text lines
                ...
                # lower than common line spacing: needn't to create new text block
                    ...
                    ref_line, block
                ) <= ref_dis + 1.0 and ref_dis <= max_line_spacing_ratio * line_height(ref_line):
                    start_new_block = False
    ...
    @staticmethod
    def _split_text_block_vertically(
        instances: list,
        line_break_free_space_ratio: float,
        new_paragraph_free_space_ratio: float,
    ):
        ...
        for block in instances:
            ...
            lines_list = block.lines.split_vertically_by_text(
                line_break_free_space_ratio, new_paragraph_free_space_ratio
            )
            if len(lines_list) == 1:
                blocks.append(block)
            else:
    ...
    def _parse_block_vertical_spacing(self):
        ...
        if block.is_table_block:
            block[-1].height -= constants.MINOR_DIST
    ...
    def _parse_line_spacing(self):
        ...
        def is_exact_line_spacing(block):
            ...
            for line in block.lines:
                absent_line_heights = list(
                    not span.is_valid_line_height for span in line.spans if isinstance(span, TextSpan)
                )
                if any(absent_line_heights):
                    return True
            return False
