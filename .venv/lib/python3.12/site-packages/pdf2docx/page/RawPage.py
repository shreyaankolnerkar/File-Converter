...
class RawPage(BasePage, ABC):
    ...
    def clean_up(self, **settings):
        ...
        * detect semantic type of shapes
        ...
        self.blocks.clean_up(settings["float_image_ignorable_gap"], settings["line_overlap_threshold"])
        # clean up shapes
        self.shapes.clean_up(settings["max_border_width"], settings["shape_min_dimension"])
        return self.shapes
    ...
    def parse_section(self, **settings):
        ...
        for row in elements.group_by_rows():
            ...
            if pre_num_col == 2 and current_num_col == 1:
                ...
                combine.extend(row)
                if len(combine.group_by_columns(sorted=False)) == 1:
                    current_num_col = 1
