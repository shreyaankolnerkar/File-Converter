...
class RawPageFitz(RawPage):
    ...
    def _preprocess_text(self, **settings):
        ...
        raw = self.page_engine.get_text(
            "rawdict",
            flags=0 | fitz.TEXT_MEDIABOX_CLIP | fitz.TEXT_CID_FOR_UNKNOWN_UNICODE,
            sort=sort,
        )
            ...
            for line in block["lines"]:
                for span in line["spans"]:
                    for filter_span in filtered_spans:
                        ...
                        if (
                            intersected_area / span_area(span["bbox"]) >= FACTOR_A_HALF
                            and span["font"] == filter_span["font"]
                        ):
                            intersected = True
                            break
                    if intersected:
                        break  # skip further span check if found
                if intersected:
                    break  # skip further line check
    ...
    def _preprocess_hyperlinks(self):
        ...
        for link in self.page_engine.get_links():
            if link["kind"] != 2:
                continue  # consider internet address only
            hyperlinks.append(
                {
