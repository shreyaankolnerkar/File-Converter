...
class Converter:
    ...
    * Finally, generate docx with ``python-docx``.
    ...
    def __init__(self, pdf_file: str = None, password: str = None, stream: bytes = None):
    ...
    def parse(self, start: int = 0, end: int = None, pages: list = None, **kwargs):
        ...
        * parse specified pages, e.g. paragraph, image and table
        ...
        """
        return self.load_pages(start, end, pages).parse_document(**kwargs).parse_pages(**kwargs)
    ...
    def make_docx(self, filename_or_stream=None, **kwargs):
        ...
        for i, page in enumerate(parsed_pages, start=1):
            if not page.finalized:
                continue  # ignore unparsed pages
            pid = page.id + 1
            logging.info("(%d/%d) Page %d", i, num_pages, pid)
            try:
    ...
    def debug_page(
        ...
        **kwargs,
        ...
        kwargs.update({"debug": True, "debug_doc": fitz.Document(), "debug_filename": debug_pdf})
