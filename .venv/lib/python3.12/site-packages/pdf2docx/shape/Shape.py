...
class Shape(Element):
    ...
    def store(self):
        res = super().store()
        res.update({"type": self._type, "color": self.color})
        return res
    ...
    def parse_semantic_type(self, blocks: list):
        ...
        for line in blocks:
            # not intersect yet
            if line.bbox.y1 < self.bbox.y0:
                continue

            # no intersection any more
            if line.bbox.y0 > self.bbox.y1:
                break
            ...
            if rect_type != self.default_type:
                break
...
class Stroke(Shape):
    ...
    @property
    def horizontal(self):
        return abs(self._start[1] - self._end[1]) < 1e-3

    @property
    def vertical(self):
        return abs(self._start[0] - self._end[0]) < 1e-3
    ...
    def store(self):
        res = super().store()
        res.update({"start": tuple(self._start), "end": tuple(self._end), "width": self.width})
        return res
