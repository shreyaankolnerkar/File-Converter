...
class Shapes(ElementCollection):
    ...
    def strokes(self):
        ...
        instances = list(filter(lambda shape: isinstance(shape, Stroke), self._instances))
    ...
    def fillings(self):
        ...
        instances = list(
            filter(
                lambda shape: isinstance(shape, Fill) and shape.color != share.rgb_value((1, 1, 1)),
                self._instances,
            )
        )
        return Shapes(instances)
    ...
    def hyperlinks(self):
        """Hyperlink Shapes."""
        instances = list(filter(lambda shape: isinstance(shape, Hyperlink), self._instances))
        return Shapes(instances)

    @property
    def table_strokes(self):
        """Potential table borders."""
        instances = list(filter(lambda shape: shape.has_potential_type(RectType.BORDER), self._instances))
