...
class CellStructure:
    ...
    @property
    def is_merged(self):
        return self.merged_cells[0] == 0 or self.merged_cells[1] == 0

    @property
    def is_merging(self):
        return self.merged_cells[0] > 1 or self.merged_cells[1] > 1
    ...
    def _get_border_stroke(self, strokes: Shapes, direction: str = "row"):
        ...
        if len(border_strokes) == 1:
            return border_strokes[0]
...
class TableStructure:
    ...
    def __init__(self, strokes: Shapes, **settings):
        ...
        self.h_strokes, self.v_strokes = TableStructure._group_h_v_strokes(
            strokes, settings["min_border_clearance"], settings["max_border_width"]
        )
        if not self.h_strokes or not self.v_strokes:
            return
    ...
    def _finalize_strokes_fills(self):
        ...
        for row in self.cells:
            for cell in row:
                if cell.shading:
                    cell.shading.type = RectType.SHADING
    ...
    def _group_h_v_strokes(strokes: Shapes, min_border_clearance: float, max_border_width: float):
        ...
        def group_strokes(stroke: Shape, strokes: dict):
            ...
            for t_ in strokes:
                if abs(t - t_) > min_border_clearance:
                    continue
                t = (t_ + t) / 2.0  # average
        ...
        if not h_strokes or not v_strokes:
            return None, None
        ...
        for _, borders in h_strokes.items():
            borders.sort_in_line_order()
        for _, borders in v_strokes.items():
            borders.sort_in_reading_order()
    ...
    def _check_outer_strokes(table_bbox: Element, borders: dict, direction: str, max_border_width: float):
        ...
        # otherwise, check border segments
            ...
            occupied = [(border.bbox[idx_start], border.bbox[idx_end]) for border in borders[current]]
