...
class Cell(Layout):
    ...
    def make_docx(self, table, indexes):
        ...
        if n_row * n_col != 1 and ((i + n_row - 1) * table._column_count + j + n_col - 1) < len(
            table._cells
        ):  # check whether index is over length of cells
            ...
            try:
                ...
                raise Exception(
                    f"Failed to merge docx_cell={show(docx_cell)} _cell={show(_cell)}. {i=} {j=} {n_row=} {n_col=}"
                ) from e
