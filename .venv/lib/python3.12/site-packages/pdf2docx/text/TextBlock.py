...
from ..common import constants, docx
from ..common.Block import Block
from ..common.share import RectType, TextAlignment, lower_round, rgb_component_from_name
...
class TextBlock(Block):
    ...
    def store(self):
        res = super().store()
        res.update({"lines": self.lines.store()})
        return res
    ...
    def parse_text_format(self, shapes):
        ...
        for shape in shapes:
            ...
            if not shape.equal_to_type(RectType.HYPERLINK) and shape.is_determined:
                continue

            # any intersection with current block?
            if not self.bbox.intersects(shape.bbox):
                continue
    ...
    def parse_horizontal_spacing(
        ...
        **LEFT** alignment is set, and use ``TAB`` to position each line.
        ...
        # text alignment
        self.alignment = self._parse_alignment(
            bbox,
            (idx0, idx1, f),
    ...
    def parse_relative_line_spacing(self):
        ...
        if len(rows) > 1:
            line_space = max(line_space, constants.DEFAULT_LINE_SPACING)
    ...
    def _parse_alignment(
        ...
        for row in rows:
            if len(row) == 1:
                continue
            dis = [
                (row[i].bbox[idx0] - row[i - 1].bbox[idx1]) * f >= line_separate_threshold for i in range(1, len(row))
            ]
            if any(dis):
