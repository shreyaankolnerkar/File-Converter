...
class Spans(ElementCollection):
    ...
    def text_spans(self):
        ...
        spans = list(filter(lambda span: isinstance(span, TextSpan), self._instances))
        return Spans(spans)

    @property
    def image_spans(self):
        """Get ImageSpan instances."""
        spans = list(filter(lambda span: isinstance(span, ImageSpan), self._instances))
        return Spans(spans)
    ...
    def strip(self):
        ...
        if isinstance(left_span, TextSpan):
            stripped = stripped or left_span.lstrip()

        # right strip the last span
        right_span = self._instances[-1]
        if isinstance(right_span, TextSpan):
            stripped = stripped or right_span.rstrip()

        # update bbox
        if stripped:
            self._parent.update_bbox(self.bbox)
