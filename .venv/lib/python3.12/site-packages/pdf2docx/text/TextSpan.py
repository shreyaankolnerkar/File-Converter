...
from ..common.share import RectType, decode, rgb_component, rgb_value
...
class TextSpan(Element):
    ...
    def lstrip(self):
        ...
        if not original_text.startswith(" " * 2):
            return False
    ...
    def rstrip(self):
        ...
        original_text = self.text
        if not original_text.endswith(" " * 2):
            return False
    ...
    def plot(self, page, color: tuple):
        super().plot(page, stroke=color, fill=color, width=0)
    ...
    def _parse_text_format(self, rect: Shape, horizontal: bool = True):
        ...
        if rect.equal_to_type(RectType.HYPERLINK):
            self.style.append({"type": rect.type, "color": rect.color, "uri": rect.uri})
            return True
        ...
        if not rect.is_determined:
            return False

        style = {"type": rect.type, "color": rect.color}
