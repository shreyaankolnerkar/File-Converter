...
def show(args):
    ...
    if n > 0:
        s = doc.get_sigflags()
        pymupdf.message("document contains %i root form fields and is %ssigned" % (n, "not " if s != 3 else ""))
    n = doc.embfile_count()
    if n > 0:
...
def clean(args):
    ...
    encrypt = ("keep", "none", "rc4-40", "rc4-128", "aes-128", "aes-256").index(encryption)
...
def embedded_upd(args):
    ...
    if args.path is not None and os.path.exists(args.path) and os.path.isfile(args.path):
        with open(args.path, "rb") as f:
            stream = f.read()
    else:
        stream = None
    ...
    doc.embfile_upd(args.name, stream, filename=filename, ufilename=ufilename, desc=desc)
...
def embedded_list(args):
    ...
    if args.name is not None:
        if args.name not in names:
            ...
            pymupdf.message()
            pymupdf.message("printing 1 of %i embedded file%s:" % (len(names), "s" if len(names) > 1 else ""))
            pymupdf.message()
            ...
            for item in itemlist:
                ...
                if xref not in image_xrefs:
                    ...
                    if type(pix) is dict:
                        ...
                        pix2 = pix if pix.colorspace.n < 4 else pymupdf.Pixmap(pymupdf.csRGB, pix)
                        pix2.save(outname)
...
def main():
    ...
    subps = parser.add_subparsers(title="Subcommands", help="Enter 'command -h' for subcommand specific help")
    ...
    # 'clean' command
    # -------------------------------------------------------------------------
    ps_clean = subps.add_parser("clean", description=mycenter("optimize PDF, or create sub-PDF if pages given"))
    ...
    ps_clean.add_argument("-ascii", action="store_true", default=False, help="ASCII encode binary data")
    ...
    ps_clean.add_argument("-pretty", action="store_true", default=False, help="prettify PDF structure")
    ...
    # 'extract' command
    # -------------------------------------------------------------------------
    ps_extract = subps.add_parser("extract", description=mycenter("extract images and fonts to disk"))
    ...
    ps_extract.add_argument("-pages", type=str, help="consider these pages only, format: 1,5-7,50-N")
    ps_extract.set_defaults(func=extract_objects)

    # -------------------------------------------------------------------------
    # 'embed-info'
    # -------------------------------------------------------------------------
    ps_show = subps.add_parser("embed-info", description=mycenter("list embedded files"))
    ...
    # 'embed-add' command
    # -------------------------------------------------------------------------
    ps_embed_add = subps.add_parser("embed-add", description=mycenter("add embedded file"))
    ps_embed_add.add_argument("input", help="PDF filename")
    ps_embed_add.add_argument("-password", help="password")
    ps_embed_add.add_argument("-output", help="output PDF filename, incremental save if none")
    ...
    # 'embed-del' command
    # -------------------------------------------------------------------------
    ps_embed_del = subps.add_parser("embed-del", description=mycenter("delete embedded file"))
    ps_embed_del.add_argument("input", help="PDF filename")
    ps_embed_del.add_argument("-password", help="password")
    ps_embed_del.add_argument("-output", help="output PDF filename, incremental save if none")
    ...
    ps_embed_upd.add_argument("-output", help="Output PDF filename, incremental save if none")
    ...
    ps_embed_upd.add_argument("-ufilename", help="new unicode filename to store in entry")
    ...
    # -------------------------------------------------------------------------
    ps_embed_extract = subps.add_parser("embed-extract", description=mycenter("extract embedded file to disk"))
    ...
    ps_embed_extract.add_argument("-output", help="output filename, default is stored name")
    ps_embed_extract.set_defaults(func=embedded_get)

    # -------------------------------------------------------------------------
    # 'embed-copy' command
    # -------------------------------------------------------------------------
    ps_embed_copy = subps.add_parser("embed-copy", description=mycenter("copy embedded files between PDFs"))
    ...
    ps_embed_copy.add_argument("-output", help="output PDF, incremental save to 'input' if omitted")
    ...
    # 'textlayout' command
    # -------------------------------------------------------------------------
    ps_gettext = subps.add_parser("gettext", description=mycenter("extract text in various formatting modes"))
    ...
    # '_internal' command
    # -------------------------------------------------------------------------
    ps_internal = subps.add_parser("internal", description=mycenter("internal testing"))
