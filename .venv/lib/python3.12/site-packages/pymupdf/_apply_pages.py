...
def _worker_fn(page_number):
    ...
    if not _worker_state.document:
        if _worker_state.stats:
            t = time.time()
        _worker_state.document = pymupdf.Document(
            _worker_state.path
        )  # pylint: disable=c-extension-no-member,attribute-defined-outside-init
        if _worker_state.stats:
    ...
    def childfn():
        ...
        while 1:
            ...
            try:
                ...
                if verbose:
                    pymupdf.log(f"{os.getpid()=}: exception {e=}")
                ret = e
            if verbose:
