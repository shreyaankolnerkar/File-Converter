...
def get_text_words(
    ...
    def sort_words(words):
        ...
        for w in words[1:]:
            ...
            if abs(wrect.y0 - lrect.y0) <= tolerance or abs(wrect.y1 - lrect.y1) <= tolerance:
                line.append(w)
                lrect |= wrect
            else:
    ...
    if textpage is not None and clip is not None:
        ...
        words = [w for w in words if abs(clip & w[:4]) >= 0.5 * abs(pymupdf.Rect(w[:4]))]
...
def get_text(
    ...
    *,
    ...
    if option == "blocks":
        return get_text_blocks(page, clip=clip, flags=flags, textpage=textpage, sort=sort)
...
def getDestStr(xref: int, ddict: dict) -> str:
    ...
    str_gotor1 = (
        lambda a, b, c, d, e, f: f"/A<</S/GoToR/D[{a} /XYZ {_format_g((b, c, d))}]/F<</F{e}/UF{f}/Type/Filespec>>>>"
    )
