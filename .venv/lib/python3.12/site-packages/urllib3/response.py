...
try:
    ...
    class ZstdDecoder(ContentDecoder):
        ...
        def decompress(self, data: bytes, max_length: int = -1) -> bytes:
            ...
            while self._obj.eof and self._obj.unused_data and (max_length < 0 or length < max_length):
...
class MultiDecoder(ContentDecoder):
    ...
    def __init__(self, modes: str) -> None:
        ...
        if len(encodings) > self.max_decode_links:
            raise DecodeError("Too many content encodings in the chain: " f"{len(encodings)} > {self.max_decode_links}")
...
class HTTPResponse(BaseHTTPResponse):
    ...
    def read(
        ...
        if not data and len(self._decoded_buffer) == 0 and not (self._decoder and self._decoder.has_unconsumed_tail):
            return data
    ...
    def read1(
        ...
        if self._has_decoded_content:
            ...
            if self._decoder and self._decoder.has_unconsumed_tail and (amt is None or len(self._decoded_buffer) < amt):
                decoded_data = self._decode(
                    b"",
                    decode_content,
                    flush_decoder=False,
    ...
    def shutdown(self) -> None:
        ...
        if self._connection is None:
            raise RuntimeError("Cannot shutdown as connection has already been released to the pool")
    ...
    def read_chunked(self, amt: int | None = None, decode_content: bool | None = None) -> typing.Generator[bytes]:
        """
