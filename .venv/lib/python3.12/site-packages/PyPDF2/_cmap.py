...
) -> Tuple[str, float, Union[str, Dict[int, str]], Dict, DictionaryObject]:  # font_type,space_width /2, encoding, cmap
...
def compute_space_width(ft: DictionaryObject, space_code: int, space_width: float) -> float:
    ...
    if "/DescendantFonts" in ft:  # ft["/Subtype"].startswith("/CIDFontType"):
        ...
        try:
            ...
            sp_width = w1[-1] / 2.0  # if using default we consider space will be only half size
    elif "/Widths" in ft:
